...
### PURPOSE: Decompose the total effect into the direct and indirect effects
  ...
  # Code Blocks
    ...
    # 4. Tidy effect decomposition
    # 5. Graph effect decomposition
...
###############################################################################
  ...
  ### 4.1 Set standard values for the tidy tibbles
    ## a) a list of effect types
      `Effect type` <- c('indirect', 'direct', 'total', 'Prop. Mediated')
     

  ### 4.2 Tidy female effects decomposition: female age + strength
      # with total fecundity.
  ...
  ### 4.3 Tidy female effects decomposition: female tail streamer length +
...
###############################################################################
##############           5. Graph effects decomposition            ############
############################################################################### 
      
  ### 5.1 Female effects decomposition graphs
  ...
  ## c) Graph results of total, direct and indirect effects from model of 
      ...
      f_TS_age_strength_fecund_plot <- 
        ggplot(data = (tidy_f_TS_age_strength_fecund %>%
                         filter(`Effect type` != 'Prop. Mediated')), 
               aes(x = `Effect type`, 
                   y = Estimate, 
                   color = `Effect type`)) +
